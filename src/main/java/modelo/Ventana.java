...
public class Ventana extends JFrame {
    ...
    //iniciar variables en JLabel 
    tabla = new DefaultTableModel();
    tabla.addColum("nomblreL");
    tabla.addColum("matriculaL");
    tabla.addColum("carreraL");
    tabla.addColum("correoL");
    this.JTable.setModelo(tabla);
    ...
    int cantidadfilas JTable.getRow1count();
    for (int i=cantidadfilas-1;i>=0;1--){
    modelo.removeRow(i);
}
    private final JButton agregar;{
   String [] Datos = new String (4);
    Datos[0]=jTextField nombreTF.getText();
    jTexField nommbreTF.serText(null);
    Datos [1]=jTextField matriculaTF.getText();
    jTexField matriculaTF.serText(null);
    Datos [2]=jTextField carreraTF.getText();
    jTexField carreraTF.serText(null);
    Datos [3]=jTextField correoTF.getText();
    jTexField correoTF.serText(null);
    tabla.addRow(datos);
}
    private final JButton eliminar;static {
        int filaselecionada= JTable.getSelectedRow();
        if (filaselecionada>==0 ){
        tabla.removeRow(filaseleccionada);
        }
        else {
         JOptionPane.showMessageDialog(null, "tabla vacia o no se ha elegido una fila");
        }
